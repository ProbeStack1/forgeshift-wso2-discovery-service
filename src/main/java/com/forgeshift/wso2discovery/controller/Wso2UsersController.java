...
/**
 * POST /wso2/users - per-resource discovery for WSO2 users from the SCIM 2.0
 * user store. Requires the OAuth client to be granted SCIM read scope
 * (typically rolled into apim:admin in WSO2 4.x).
 */
@Slf4j
@RestController
@RequestMapping("/wso2")
@RequiredArgsConstructor
public class Wso2UsersController {

    private final Wso2UsersDiscoveryService service;
    ...
    public ResponseEntity<DiscoverResourceResponse> discover(@Valid @RequestBody DiscoverResourceRequest req) {
        applyDefaults(req);
        log.info("POST /wso2/users company={} tenant={} env={} txn={}",
