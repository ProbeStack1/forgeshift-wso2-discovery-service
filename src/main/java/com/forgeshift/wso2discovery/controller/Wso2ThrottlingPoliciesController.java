...
/**
 * POST /wso2/throttlingpolicies - per-resource discovery for Throttling Policies.
 * Merges subscription + application + advanced tier families.
 */
@Slf4j
@RestController
@RequestMapping("/wso2")
@RequiredArgsConstructor
public class Wso2ThrottlingPoliciesController {

    private final Wso2ThrottlingPoliciesDiscoveryService service;
    private final DiscoveryProperties discoveryProps;

    @PostMapping("/throttlingpolicies")
    public ResponseEntity<DiscoverResourceResponse> discover(@Valid @RequestBody DiscoverResourceRequest req) {
        applyDefaults(req);
        log.info("POST /wso2/throttlingpolicies company={} tenant={} env={} txn={}",
