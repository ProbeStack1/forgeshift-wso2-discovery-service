...
/**
 * POST /wso2/keymanagers - per-resource discovery for registered Key Managers.
 */
@Slf4j
@RestController
@RequestMapping("/wso2")
@RequiredArgsConstructor
public class Wso2KeyManagersController {

    private final Wso2KeyManagersDiscoveryService service;
    private final DiscoveryProperties discoveryProps;

    @PostMapping("/keymanagers")
    public ResponseEntity<DiscoverResourceResponse> discover(@Valid @RequestBody DiscoverResourceRequest req) {
        applyDefaults(req);
        log.info("POST /wso2/keymanagers company={} tenant={} env={} txn={}",
